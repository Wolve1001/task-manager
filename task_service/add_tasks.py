...
def add_task(user_name: str, task_name: str, due_date: date, recurring: str = None, notify_at: datetime = None, category: str = None):
    try:
        ...
        if due_date >= date.today() and notify_at.time() > datetime.now().time():
            cursor.execute("INSERT INTO user_info.tasks (user_name, task_name, due_date, status, repeat, notify_at, category) VALUES (%s, %s, %s, 'pending', %s, %s, %s)", (user_name, task_name, due_date, recurring, notify_at, category))
        else:
